...
def printfn():
    ...
    rep=str(ws_input.cell(row=2, column=4).value)
    inter=str(ws_input.cell(row=3, column=4).value)
    bench=str(ws_input.cell(row=4, column=4).value)
    if (inter=="1wk"):
        interval=52
        lab="Weeks"
    if (inter=="1mo"):
    ...
    def pflio(DF,num):
        ...
        for i in range(1,len(df)):
            if len(portfolio) > 0:
                ...
                rem_stocks = df[portfolio].iloc[i,:].sort_values(ascending=True)[:num].index.values.tolist()
    ...
    CAGR(pflio(return_df,num))
    sharpe(pflio(return_df,num),0.025)
    max_dd(pflio(return_df,num)) 
    ...
    #plot
    fig, ax = plt.subplots()
    plt.plot((1+pflio(return_df,num)).cumprod())
    ...
    fig.savefig('Backtest - Swing portfolio.png')
    img=Image('Backtest - Swing portfolio.png')
    img.height=400
    img.width=600
